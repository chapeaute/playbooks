...
- cassandra_grant: permission='select' all_keyspaces=True role=read_only login_host=localhost login_pass=cassandra login_user=cassandra

# Revoke modify permission to foo keyspace
- cassandra_grant: mode=revoke permission=modify keyspace=foo role=no_modify_foo login_host=localhost login_pass=cassandra login_user=cassandra

# Inherit roles
- cassandra_grant: mode=grant inherit_role=read_only role=my_user_role login_host=localhost login_pass=cassandra login_user=cassandra
'''
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            login_user=dict(default='cassandra'),
            login_password=dict(default='cassandra', no_log=True),
            login_host=dict(default='localhost'),
            login_port=dict(default=9042),
            protocol_version=dict(default=3),
            state=dict(default='present', choices=['present', 'absent']),
            permission=dict(default="all", requried=False, choices=["all", "create", "alter", "drop", "select", "modify", "authorize"]),
            role=dict(required=True, aliases=['user']),
            inherit_role=dict(default=None, required=False),
            keyspace=dict(default=None, required=True),
            all_keyspaces=dict(default=None, required=False)
            )
        )
    ...
    try:
        if not login_user:
            cluster = Cluster(login_host, port=login_port)
        else:
            auth_provider = PlainTextAuthProvider(username=login_user, password=login_password)
            cluster = Cluster([login_host], auth_provider=auth_provider, protocol_version=4, port=login_port)
